...
func TestApiV1Convert(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create a mock image convert service that always returns a successful result
			mockImageConvertService := &MockImageConvertService{}
			
			convertUsecase := usecase.NewConvertUsecase(fileBuilder, mockImageConvertService)
